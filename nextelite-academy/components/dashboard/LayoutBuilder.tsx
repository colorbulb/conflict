  ...
  const renderBlock = (block: LayoutBlock) => {
    return (
      <div key={block.id} className="bg-white border-2 border-gray-200 rounded-xl p-6 mb-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-bold text-gray-700 capitalize">
              {block.type === 'text-image' ? 'Text Left, Image Right' : 
               ...
               block.type === 'image-text-stack' ? 'Image Top, Text Bottom' :
               'Text Only'}
            </span>
          </div>
          <button
        ...
        {block.type === 'image-text-stack' && (
          <div className="space-y-4">
            <div>
              <label className="block text-xs font-bold text-gray-700 mb-1">Image (Top)</label>
              {block.imageUrl && (
                <div className="mb-2 rounded-lg overflow-hidden border border-gray-200 h-32 bg-gray-100">
                  <img src={block.imageUrl} alt="Block" className="w-full h-full object-cover" />
                </div>
              ...
              <input
                ...
                className="hidden"
                id={`image-stack-upload-${block.id}`}
              />
              <label
                htmlFor={`image-stack-upload-${block.id}`}
                className="mt-1 block text-center text-xs text-brand-blue hover:text-brand-purple font-bold cursor-pointer"
              >
                Or click to browse
              </label>
            ...
            <div>
              <label className="block text-xs font-bold text-gray-700 mb-1">Text Content (Bottom)</label>
              <textarea
                value={block.text}
                onChange={(e) => updateBlock(block.id, { text: e.target.value })}
                className="w-full border rounded-lg p-3 text-sm h-32 focus:ring-2 focus:ring-brand-blue outline-none"
    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={() => addBlock('image-text-stack')}
            className="text-xs bg-brand-orange text-white px-3 py-1.5 rounded-lg font-bold hover:bg-orange-600 flex items-center gap-1"
          >
            <Plus className="w-3 h-3" /> Image Top, Text Bottom
          </button>
        </div>
      </div>
