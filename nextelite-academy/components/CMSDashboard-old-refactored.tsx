...
const CMSDashboard: React.FC<CMSDashboardProps> = ({
  ...
  const [courseEditingLang, setCourseEditingLang] = useState<'en' | 'zh'>('en');
  const [instructorEditingLang, setInstructorEditingLang] = useState<'en' | 'zh'>('en');
  const [aboutEditingLang, setAboutEditingLang] = useState<'en' | 'zh'>('en');
  ...
  const [localEnPageContent, setLocalEnPageContent] = useState<PageContent>(enPageContent);
  const [localZhPageContent, setLocalZhPageContent] = useState<PageContent>(zhPageContent);
  ...
  // Update local page content when props change
  useEffect(() => {
    setLocalEnPageContent(enPageContent);
  }, [enPageContent]);
  
  useEffect(() => {
    setLocalZhPageContent(zhPageContent);
  }, [zhPageContent]);
  ...
  // Get current page content and menu items based on editing language
  const currentAboutPageContent = aboutEditingLang === 'en' ? localEnPageContent : localZhPageContent;
  const setCurrentAboutPageContent = aboutEditingLang === 'en' ? setLocalEnPageContent : setLocalZhPageContent;
  ...
  const handleSavePageContent = () => {
      ...
      onUpdatePageContent(localEnPageContent, 'en');
      onUpdatePageContent(localZhPageContent, 'zh');
  ...
  const updateCourseOutline = (idx: number, val: string) => {
      ...
      const lang = courseEditingLang;
      const newOutline = [...editingCourse[lang].outline];
      newOutline[idx] = val;
      setEditingCourse({
          ...editingCourse,
          [lang]: {
  ...
  const addOutlineItem = () => {
      ...
      setEditingCourse({
          ...
          [lang]: {
              ...editingCourse[lang],
              outline: [...editingCourse[lang].outline, 'New Module']
          }
      });
  };
  const removeOutlineItem = (idx: number) => {
      ...
      setEditingCourse({
          ...
          [lang]: {
              ...editingCourse[lang],
              outline: editingCourse[lang].outline.filter((_, i) => i !== idx)
          }
      });
  };
      ...
      return (
        <div className="p-6 max-w-5xl mx-auto">
            ...
            <div className="bg-white rounded-3xl shadow-xl overflow-hidden border border-gray-100">
                <div className={`p-8 border-b border-gray-100 ${editingCourse.color} text-white`}>
                    ...
                    <div className="flex gap-2 bg-white/20 p-1 rounded-lg w-fit">
                        <button
                            onClick={() => setCourseEditingLang('en')}
                            className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${
                                courseEditingLang === 'en'
                        ...
                        <button
                            onClick={() => setCourseEditingLang('zh')}
                            className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${
                                courseEditingLang === 'zh'
                                    ? 'bg-white text-brand-blue shadow-sm'
                                    : 'text-white/80 hover:text-white'
                            }`}
                        >
                ...
                <div className="p-8 space-y-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input 
                                value={editingCourse[courseEditingLang].title}
                                onChange={(e) => setEditingCourse({
                                    ...editingCourse,
                                    [courseEditingLang]: {
                                        ...editingCourse[courseEditingLang],
                        ...
                        <div className="md:col-span-2">
                             ...
                             <textarea 
                                value={editingCourse[courseEditingLang].description}
                                onChange={(e) => setEditingCourse({
                                    ...editingCourse,
                                    [courseEditingLang]: {
                                        ...editingCourse[courseEditingLang],
                        ...
                        <div className="md:col-span-2">
                             ...
                             <RichTextEditor
                                initialValue={editingCourse[courseEditingLang].fullDescription || ''}
                    ...
                    <div>
                        ...
                        <div className="space-y-2">
                            {editingCourse[courseEditingLang].outline.map((item, idx) => (
                    ...
                    <div>
                                ...
                                if (files.length > 0) {
                                    try {
                                        ...
                                        setEditingCourse({
                                            ...
                                            [courseEditingLang]: {
                                                ...editingCourse[courseEditingLang],
                                                attachments: [...editingCourse[courseEditingLang].attachments, ...results]
                                            }
                                        });
                        ...
                        {editingCourse[courseEditingLang].attachments.length > 0 && (
                            <div className="mt-4 space-y-2">
                                {editingCourse[courseEditingLang].attachments.map((att, idx) => (
                                                ...
                                                onClick={() => {
                                                    if (confirm('Delete this attachment?')) {
                                                        setEditingCourse({
                                                            ...
                                                            [courseEditingLang]: {
                                                                ...editingCourse[courseEditingLang],
                                                                attachments: editingCourse[courseEditingLang].attachments.filter((_, i) => i !== idx)
                                                            }
                                                        });
                                                    }
                                                }}
                    ...
                    <div>
                                ...
                                if (files.length > 0) {
                                    try {
                                        ...
                                        setEditingCourse({
                                            ...
                                            [courseEditingLang]: {
                                                ...editingCourse[courseEditingLang],
                                                galleryImages: [...(editingCourse[courseEditingLang].galleryImages || []), ...newUrls]
                                            }
                                        });
                        ...
                        {editingCourse[courseEditingLang].galleryImages && editingCourse[courseEditingLang].galleryImages.length > 0 && (
                            <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-4">
                                {editingCourse[courseEditingLang].galleryImages.map((imgUrl, idx) => (
                                            ...
                                            onClick={() => {
                                                if (confirm('Delete this image?')) {
                                                    setEditingCourse({
                                                        ...
                                                        [courseEditingLang]: {
                                                            ...editingCourse[courseEditingLang],
                                                            galleryImages: editingCourse[courseEditingLang].galleryImages?.filter((_, i) => i !== idx) || []
                                                        }
                                                    });
                                                }
                                            }}
                    ...
                    <div>
                        <div className="flex justify-between items-center mb-2">
                            ...
                            >
                                {editingCourse[courseEditingLang].quiz ? <X className="w-3 h-3" /> : <Plus className="w-3 h-3" />}
                                {editingCourse[courseEditingLang].quiz ? 'Remove Quiz' : 'Add Quiz'}
                            </button>
                        </div>
                        
                        {editingCourse[courseEditingLang].quiz && (
                            <div className="bg-gray-50 rounded-xl p-6 border border-gray-200 space-y-6">
                                <div>
                                    ...
                                    <input
                                        type="text"
                                        value={editingCourse[courseEditingLang].quiz.title}
                                        onChange={(e) => {
                                            const lang = courseEditingLang;
                                            setEditingCourse({
                                                ...editingCourse,
                                ...
                                <div>
                                                ...
                                                setEditingCourse({
                                                    ...
                                                    [lang]: {
                                                        ...
                                                        quiz: {
                                                            ...editingCourse[lang].quiz!,
                                                            questions: [
                                                                ...editingCourse[lang].quiz!.questions,
                                                                {
                                                                    question: '',
                                                                    options: ['', '', '', ''],
                                                                    correctIndex: 0
                                    ...
                                    <div className="space-y-4">
                                        {editingCourse[courseEditingLang].quiz.questions.map((q, qIdx) => (
                                            <div key={qIdx} className="bg-white p-4 rounded-lg border border-gray-200">
                                                            ...
                                                            if (confirm('Delete this question?')) {
                                                                ...
                                                                setEditingCourse({
                                                                    ...
                                                                    [lang]: {
                                                                        ...
                                                                        quiz: {
                                                                            ...editingCourse[lang].quiz!,
                                                                            questions: editingCourse[lang].quiz!.questions.filter((_, i) => i !== qIdx)
                                                                        }
                                                                    }
                                                                });
                                                            }
                                                ...
                                                <input
                                                    ...
                                                    onChange={(e) => {
                                                        const lang = courseEditingLang;
                                                        const newQuestions = [...editingCourse[lang].quiz!.questions];
                                    ...
                                    {editingCourse[courseEditingLang].quiz.questions.length === 0 && (
  ...
  if (editingPage) {
      return (
        <div className="p-6 max-w-5xl mx-auto">
            <button onClick={() => setEditingPage(null)} className="flex items-center text-gray-500 hover:text-gray-800 mb-6 font-bold">
                        ...
                        <button 
                            onClick={async () => {
                                ...
                                if (!editingPage.translations.en.name.trim() || !editingPage.translations.zh.name.trim()) {
                                    alert('Please enter a page name for both languages');
                                    return;
                                }
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3">
                    <div className="p-8 col-span-2 space-y-6">
                         <div>
                            ...
                            <input 
                                value={editingPage.translations[pageEditingLang].name}
                                onChange={(e) => setEditingPage({
                                    ...editingPage,
                                    translations: {
                                        ...editingPage.translations,
                                        [pageEditingLang]: {
                                            ...editingPage.translations[pageEditingLang],
                                            name: e.target.value
                                        }
                                    }
                                })}
                         ...
                         <div>
                             <div className="flex justify-between items-center mb-2">
                                 ...
                                 <div className="flex gap-2">
                                     <button
                                         onClick={() => {
                                             const useLayout = !editingPage.translations[pageEditingLang].layoutBlocks;
                                             setEditingPage({
                                                 ...editingPage,
                                                 translations: {
                                                     ...editingPage.translations,
                                                     [pageEditingLang]: {
                                                         ...
                                                         content: useLayout ? '' : (editingPage.translations[pageEditingLang].content || '')
                                                     }
                                                 }
                                             });
                                         }}
                                         className={`text-xs px-3 py-1 rounded-lg font-bold transition-colors ${
                                             editingPage.translations[pageEditingLang].layoutBlocks !== undefined
                                                 ? 'bg-brand-purple text-white' 
                                                 : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
                                         }`}
                                     >
                                         {editingPage.translations[pageEditingLang].layoutBlocks !== undefined ? 'Using Layout Builder' : 'Switch to Layout Builder'}
                                     </button>
                                 </div>
                             </div>
                             ...
                             {editingPage.translations[pageEditingLang].layoutBlocks !== undefined ? (
                                 <LayoutBuilder
                                     blocks={editingPage.translations[pageEditingLang].layoutBlocks || []}
                                     onChange={(blocks) => {
                                 ...
                                 <RichTextEditor 
                                    key={`${editingPage.id}-${pageEditingLang}`}
                                    initialValue={editingPage.translations[pageEditingLang].content}
        ...
        <div className="p-6 max-w-3xl mx-auto">
            ...
            <div className="bg-white rounded-3xl shadow-xl overflow-hidden border border-gray-100">
                <div className="p-8 border-b border-gray-100 bg-gray-50">
                    ...
                    <div className="flex gap-2 bg-gray-100 p-1 rounded-lg w-fit">
                        <button
                            onClick={() => setInstructorEditingLang('en')}
                            className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${
                                instructorEditingLang === 'en'
                                    ? 'bg-white text-brand-blue shadow-sm'
                                    : 'text-gray-600 hover:text-gray-800'
                            }`}
                        >
                        ...
                        <button
                            onClick={() => setInstructorEditingLang('zh')}
                            className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${
                                instructorEditingLang === 'zh'
                                    ? 'bg-white text-brand-blue shadow-sm'
                                    : 'text-gray-600 hover:text-gray-800'
                            }`}
                        >
                ...
                <div className="p-8 space-y-6">
                    <div className="flex gap-6 items-start">
                         ...
                         <div className="flex-1 space-y-4">
                             <div>
                                ...
                                <input 
                                    value={editingInstructor[instructorEditingLang].name}
                                    onChange={(e) => setEditingInstructor({
                                        ...editingInstructor,
                                        [instructorEditingLang]: {
                                            ...editingInstructor[instructorEditingLang],
                                            name: e.target.value
                                        }
                                    })}
                             ...
                             <div>
                                ...
                                <input 
                                    value={editingInstructor[instructorEditingLang].role}
                                    onChange={(e) => setEditingInstructor({
                                        ...editingInstructor,
                                        [instructorEditingLang]: {
                                            ...editingInstructor[instructorEditingLang],
                    ...
                    <div>
                        ...
                        <textarea 
                            value={editingInstructor[instructorEditingLang].bio}
                            onChange={(e) => setEditingInstructor({
                                ...editingInstructor,
                                [instructorEditingLang]: {
                                    ...editingInstructor[instructorEditingLang],
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="border-t border-gray-200 pt-8">
                            <div className="flex justify-between items-center mb-4">
                                <h4 className="text-lg font-bold text-gray-800">About Us Section</h4>
                                <div className="flex gap-2 bg-gray-100 p-1 rounded-lg">
                                    <button
                                        onClick={() => setAboutEditingLang('en')}
                                        className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${
                                            aboutEditingLang === 'en'
                                                ? 'bg-white text-brand-blue shadow-sm'
                                                : 'text-gray-600 hover:text-gray-800'
                                    ...
                                    <button
                                        onClick={() => setAboutEditingLang('zh')}
                                        className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${
                                            aboutEditingLang === 'zh'
                                                ? 'bg-white text-brand-blue shadow-sm'
                                                : 'text-gray-600 hover:text-gray-800'
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input 
                                        value={currentAboutPageContent.about?.heading || ''}
                                        onChange={(e) => setCurrentAboutPageContent({
                                            ...currentAboutPageContent,
                                            about: {
                                                ...(currentAboutPageContent.about || { subheading: '', content: '' }),
                                ...
                                <div>
                                    ...
                                    <input 
                                        value={currentAboutPageContent.about?.subheading || ''}
                                        onChange={(e) => setCurrentAboutPageContent({
                                            ...currentAboutPageContent,
                                            about: {
                                                ...(currentAboutPageContent.about || { heading: '', content: '' }),
                                ...
                                <div className="md:col-span-2">
                                    <div className="flex justify-between items-center mb-2">
                                        ...
                                        <button
                                            onClick={() => {
                                                const useLayout = !currentAboutPageContent.about?.layoutBlocks;
                                                setCurrentAboutPageContent({
                                                    ...currentAboutPageContent,
                                                    about: {
                                                        ...(currentAboutPageContent.about || { heading: '', subheading: '', content: '' }),
                                                        layoutBlocks: useLayout ? [] : undefined,
                                                        content: useLayout ? '' : (currentAboutPageContent.about?.content || '')
                                                    }
                                                });
                                            }}
                                            className={`text-xs px-3 py-1 rounded-lg font-bold transition-colors ${
                                                currentAboutPageContent.about?.layoutBlocks !== undefined
                                                    ? 'bg-brand-purple text-white' 
                                                    : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
                                            }`}
                                        >
                                            {currentAboutPageContent.about?.layoutBlocks !== undefined ? 'Using Layout Builder' : 'Switch to Layout Builder'}
                                        </button>
                                    </div>
                                    
                                    {currentAboutPageContent.about?.layoutBlocks !== undefined ? (
                                        <LayoutBuilder
                                            blocks={currentAboutPageContent.about.layoutBlocks || []}
                                            onChange={(blocks) => {
                                                // Convert layout blocks to HTML
                                                const html = blocks.map(block => {
                                                    if (block.type === 'text-image') {
                                                ...
                                                setCurrentAboutPageContent({
                                                    ...currentAboutPageContent,
                                                    about: {
                                                        ...(currentAboutPageContent.about || { heading: '', subheading: '' }),
                                                        layoutBlocks: blocks,
                                                        content: html
                                                    }
                                                });
                                        ...
                                        <RichTextEditor
                                            initialValue={currentAboutPageContent.about?.content || ''}
                                            onChange={(html) => setCurrentAboutPageContent({
                                                ...currentAboutPageContent,
                                                about: {
                                                    ...(currentAboutPageContent.about || { heading: '', subheading: '' }),
                                                    content: html
                                ...
                                <div className="md:col-span-2">
                                    ...
                                    <input 
                                        type="text"
                                        value={currentAboutPageContent.about?.imageUrl || ''}
                                        onChange={(e) => setCurrentAboutPageContent({
                                            ...currentAboutPageContent,
                                            about: {
                                                ...(currentAboutPageContent.about || { heading: '', subheading: '', content: '' }),
                                    ...
                                    />
                                    {currentAboutPageContent.about?.imageUrl && (
                                        <div className="mt-2 rounded-lg overflow-hidden border border-gray-200 h-32 bg-gray-100">
                                            <img src={currentAboutPageContent.about.imageUrl} alt="About Preview" className="w-full h-full object-cover" onError={(e) => { e.currentTarget.style.display = 'none'; }} />
                                        </div>
                                    )}
                                </div>
                            </div>
                    ...
                    {courses.map(course => (
                        <div key={course.id} className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 hover:shadow-xl transition-all group relative overflow-hidden">
                            ...
                            <div className="flex justify-between items-start mb-4 pl-4">
                                <h3 className="font-bold text-lg leading-tight">{course[language].title}</h3>
                            ...
                            <p className="text-gray-500 text-sm line-clamp-3 mb-4 pl-4">{course[language].description}</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {instructors.map(inst => (
                        <div key={inst.id} className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4 hover:shadow-lg transition-shadow">
                            <img src={inst.imageUrl} alt={inst[language].name} className="w-16 h-16 rounded-full object-cover border-2 border-brand-yellow" />
                            <div className="flex-1 min-w-0">
                                <h3 className="font-bold text-gray-800 truncate">{inst[language].name}</h3>
                                <p className="text-sm text-brand-blue font-medium truncate">{inst[language].role}</p>
                            </div>
                    ...
                    {currentCustomPages.map(page => (
                        <div key={page.id} className="bg-white p-4 rounded-2xl border border-gray-100 flex flex-col gap-4 group hover:shadow-xl transition-all">
                            <div className="flex justify-between items-start">
                                <div className="flex-1 min-w-0">
                                    <h3 className="font-bold text-gray-800 truncate">{page.translations.en.name}</h3>
